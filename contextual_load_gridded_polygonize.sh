...
&& java -Xmx10G -cp "${JAVA_CLASSPATH}" au.org.ala.layers.ingestion.contextual.ContextualFromShapefileDatabaseLoader "${LAYER_ID}" "${LAYER_SHORT_NAME}" "${LAYER_DISPLAY_NAME}" "${FIELDS_SID}" "${FIELDS_SNAME}" "${FIELDS_SDESCRIPTION}" "${REPROJECTEDSHAPEFILE}" "${DB_USERNAME}" "${DB_PASSWORD}" "${DB_JDBC_URL}" "${DB_HOST}" "${DB_NAME}" "${DERIVED_COLUMNS_FILE}" \
